var omniva_map_ready = false;
jQuery('document').ready(function($){
    console.log('omniva.js is ON');
    $(document).on('click', 'input[name="shipping_method"]', function(){
        var current_method = $(this);
        var method_value = current_method.val();
        console.log(method_value);
        if (method_value.includes('fake.parcel_terminal') || method_value.includes('omnivalt.parcel_terminal')){
            // First time during activation move selector container 
            if (omniva_map_ready) {
                $('#terminal-container').show();
            }
            else {
                // move terminal container outside <label>
                $('#terminal-container')
                    .parent().parent()
                    .append($('#terminal-container'));
                // initialize map on the terminal selector
                $('#omnivalt_parcel_terminal').omniva();
                omniva_map_ready = true;
                $('#terminal-container').show();
            }
        } else {
...
(function ( $ ) {
    $.fn.omniva = function(options) {
        ...
        omnivadata.base_url = $('base').prop('href');
        ...
        // loading overlay for selector element
        var overlay = $('<div class="loading-overlay" style="display: none;"></div>');
        select.parent().prepend(overlay);
        ...
        function searchByAddress(){
            if (selected == false){
            var postcode = '';
            var same_address_checkbox = $('#collapse-payment-address input[name="shipping_address"]');

            if (same_address_checkbox.length > 0 && same_address_checkbox.is(':checked')){
            ...
            $('#input-payment-postcode').on('change',function(){
              if (same_address_checkbox.length > 0 && same_address_checkbox.is(':checked')){
                postcode = $(this).val();
                search.val(postcode).trigger('select');
                console.log('Post code change (Billing):', postcode);
              }
            });

            $('#input-shipping-postcode').on('change',function(){
              if (same_address_checkbox.length > 0 && !same_address_checkbox.is(':checked')){
                postcode = $(this).val();
                search.val(postcode).trigger('select');
                console.log('Post code change (Shipping):', postcode);
              }
            }); 
            }
            console.log('Post code (Initial):', postcode);
        }
        ...
        function toggleDropdown(){
            if (container.hasClass('open')){
                ...
                innerContainer.show();
                innerContainer.find('.search-input').focus();
        ...
        function initMap(){
          ...
          var Icon2 = L.Icon.extend({
            ...
            terminalIcon = new Icon({iconUrl: omnivadata.base_url+'image/omniva/sasi.png'});
            homeIcon = new Icon2({iconUrl: omnivadata.base_url+'image/omniva/locator_img.png'});
        ...
        function terminalSelected(terminal,close) {
          if (close === undefined){
              ...
              // update selection (triggers actions hardcoded onto select element itself)
              select.trigger("change");
              if (close){
                $('#omnivaLtModal').hide();
            }
